"""
PhoBERT Full Fine-tuning â€” Vietnamese Toxic Comment Detection
...
MLFLOW_EXPERIMENT_NAME = os.environ.get("MLFLOW_EXPERIMENT_NAME", "viettoxic-self-learning-macro-f1")
...
DATASET_LAYOUT = os.environ.get("DATASET_LAYOUT", "plain").strip().lower()
MODEL_NAME = os.environ.get("MODEL_NAME", "vinai/phobert-base").strip()
MAX_LENGTH = 256
...
log(f"Saved model â†’ {best_model_path}")
...
if USE_TEMP_SCALING:
    ...
    log(f"ECE: {temperature_result['val_ece_before']:.4f} â†’ {temperature_result['val_ece_after']:.4f}  "
        f"Brier: {temperature_result['val_brier_before']:.4f} â†’ {temperature_result['val_brier_after']:.4f}")
...
for name, path in [
    ...
    if os.path.exists(path):
        ...
        log(f"Zipping {path} â†’ {zip_base}.zip ...")
...
print("Promotion status: pending â€” run Phase E promotion check", flush=True)
