...
def _read_kaggle_failure_log_tail(output_dir: Path, max_chars: int = 1800) -> str:
    log_files = sorted(output_dir.rglob("*.log"), key=lambda p: p.stat().st_mtime if p.exists() else 0, reverse=True)
    if not log_files:
        return ""
    ...
    for event in events:
        if not isinstance(event, dict):
            continue
        stream = str(event.get("stream_name") or "").strip()
        data = str(event.get("data") or "").strip()
        if not data:
            continue
        if stream == "stderr" or "Traceback" in data or "Error" in data or "Exception" in data:
            chunks.append(data)
    if not chunks:
        chunks = [str(event.get("data") or "").strip() for event in events if isinstance(event, dict) and event.get("data")]
    return "\n".join(chunks)[-max_chars:].strip()


def _resolve_bundle_url(batch_id: Optional[str], run_id: str) -> str:
    bundle_url = _setting("KAGGLE_REAL_BUNDLE_URL", REAL_BUNDLE_URL).strip()
    bundle_url_template = _setting("KAGGLE_REAL_BUNDLE_URL_TEMPLATE", REAL_BUNDLE_URL_TEMPLATE).strip()
    if bundle_url_template:
        try:
            return bundle_url_template.format(batch_id=batch_id or "", run_id=run_id).strip()
        except Exception:
            return bundle_url
    if "{" in bundle_url and "}" in bundle_url:
        try:
            return bundle_url.format(batch_id=batch_id or "", run_id=run_id).strip()
        except Exception:
            return bundle_url
    return bundle_url


def _embedded_phobert_train_script_preamble() -> str:
    ...
    return (
        "# Embedded PhoBERT train script copied from repo at trigger time.\n"
        "import pathlib as _viettoxic_pathlib\n"
        "_train_script_path = _viettoxic_pathlib.Path('/kaggle/working/viettoxic/scripts/train_phobert.py')\n"
        "_train_script_path.parent.mkdir(parents=True, exist_ok=True)\n"
        f"_train_script_path.write_text({json.dumps(source_text, ensure_ascii=False)}, encoding='utf-8')\n\n"
    )


def _embedded_mlflow_evidence_preamble() -> str:
    source = BASE_DIR / "kaggle" / "mlflow_evidence.py"
    if not source.exists():
        raise RuntimeError(f"Kaggle MLflow evidence module not found: {source}")
    source_text = source.read_text(encoding="utf-8-sig").replace("\ufeff", "")
    return (
        "# Embedded portable MLflow evidence contract.\n"
        "import pathlib as _viettoxic_pathlib\n"
        "import sys as _viettoxic_sys\n"
        "_evidence_module_dir = _viettoxic_pathlib.Path('/kaggle/working/viettoxic')\n"
        "_evidence_module_dir.mkdir(parents=True, exist_ok=True)\n"
        "(_evidence_module_dir / 'mlflow_evidence.py').write_text("
        f"{json.dumps(source_text, ensure_ascii=False)}, encoding='utf-8')\n"
        "if str(_evidence_module_dir) not in _viettoxic_sys.path:\n"
        "    _viettoxic_sys.path.insert(0, str(_evidence_module_dir))\n\n"
    )
...
def _build_real_script_content(payload: TriggerRequest, source_job_id: Optional[str] = None) -> str:
    ...
    env_overrides = {
        ...
        "VIETTOXIC_SOURCE_JOB_ID": source_job_id or payload.run_id,
        "VIETTOXIC_NOTEBOOK_SHA256": hashlib.sha256(source_text.encode("utf-8")).hexdigest(),
    ...
    embedded_evidence = _embedded_mlflow_evidence_preamble()
    embedded_train_script = _embedded_phobert_train_script_preamble() if requested_model_kind == "phobert" else ""
    preamble = (
...
def _fetch_existing_kernel_metadata(kernel_ref: str, job_dir: Path) -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        if "403" in lowered or "permission 'kernels.get' was denied" in lowered:
            configured_username = _setting("KAGGLE_USERNAME", "").strip()
            username_hint = f" KAGGLE_USERNAME is '{configured_username}'." if configured_username else ""
            raise RuntimeError(
                f"Cannot access Kaggle kernel '{kernel_ref}'. Check KAGGLE_KERNEL_OWNER, "
                f"KAGGLE_KERNEL_SLUG, and that the configured API token owns or can edit this kernel."
                f"{username_hint}"
            ) from exc
        return None
...
def _trigger_real(payload: TriggerRequest) -> Dict[str, Any]:
    ...
    script_content = _build_real_script_content(payload, source_job_id=job_id)
...
def _download_real_output_artifact(job_id: str) -> None:
    with _LOCK:
        state = _load_state()
        jobs = state.get("jobs", {})
    ...
    try:
        ...
        if artifact_file is None:
            raise RuntimeError(f"Kaggle completed but no output artifact found in {output_dir}")
...
def _status_real(job_id: str, job: Dict[str, Any], jobs: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if normalized == "failed":
                ...
                try:
                    ...
                    log_tail = f"Could not download Kaggle failure log: {exc}"
                detail = f"Kaggle run failed ({kernel_ref}). status={status_stdout.strip()}"
                if log_tail:
                    detail = f"{detail}\n\nKaggle log tail:\n{log_tail}"
                job["error_message"] = detail[-4000:]
            if normalized == "completed" and not job.get("artifact_uri"):
                job["status"] = "running"
                job["current_stage"] = "import_artifact"
                job["kaggle_status"] = "completed"
                if not job.get("artifact_download_started"):
                    job["artifact_download_started"] = True
                    thread = threading.Thread(target=_download_real_output_artifact, args=(job_id,), daemon=True)
                    thread.start()
    ...
    return {
        ...
        "kaggle_status": job.get("kaggle_status"),
        "artifact_uri": job.get("artifact_uri"),
        "artifact_checksum": job.get("artifact_checksum"),
        "error_message": job.get("error_message"),
    }
