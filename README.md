...
- **Cross-model re-evaluation**: an admin can run `POST /api/mlflow/re-evaluate` from the MLflow page against selected samples or all auto-eligible samples, using stored text only (no re-crawl). It writes a new immutable `model_re_evaluation` observation per `(sample, model version)` and skips a version already recorded for that sample.
- **Human authority and conflicts**: re-evaluation never reopens a `manual_accepted` sample. For an auto-accepted sample, a clear opposite model label creates `review_reason=model_conflict`; an uncertain/missing prediction creates `review_reason=model_uncertain`. Either case moves the sample to unverified candidate status, deselects it from training, and exposes it in Manual Verify. Agreement leaves the auto-label unchanged. Human review remains the only action that resolves the conflict or uncertainty.
- **Manual Verify and Training Preview**: Manual Verify lists unverified candidates, including re-evaluation conflicts/uncertainty; `Toxic`, `Clean`, and `Remove` persist the decision. Training Preview shows its selected accepted rows plus any re-evaluation rows awaiting human review, clearly flagged as not eligible for training until resolved. Bundle selection for a batch also recognizes prediction-history records from that batch, even when the canonical sample originated in an earlier batch.
