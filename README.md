...
- **Gemini-assisted review** accepts up to 25 selected rows in Manual Verify, Training Preview, and Synthetic review. The backend processes at most 3 comments per Gemini request, automatically retries an omitted/invalid multi-comment response one row at a time, and returns per-row suggestions only after all selected rows have been handled. Suggestions remain pending until the admin applies one row or uses **Apply Gemini** for all visible suggestions. Applying preserves the review origin in `training_review_status`: automatic Training Preview rows become `auto_gemini` (shown as **Tự động + Gemini**), while previously manual rows and candidate rows applied from Manual Verify become `manual_gemini` (shown as **Thủ công + Gemini**).
...
- Newly generated rows expose their current Toxic/Clean and constructiveness labels before persistence. Admins may select rows for Gemini Review, inspect each pending suggestion/confidence/reason, and apply individually or in bulk. A normal Save review persists `review_method=manual`; applying Gemini persists `review_method=gemini_assisted`. On transfer these become `manual_approved`/`synthetic_review` and `manual_gemini`/`gemini_assist`, respectively.
